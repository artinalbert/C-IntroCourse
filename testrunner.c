...
{
    StringListNode *new_node = (StringListNode *)mallocx(sizeof(StringListNode));
    new_node->item = item;
    new_node->next = *list;
...
const char **string_list_to_reversed_array(StringList list)
{
    ...
    const char **arr = (const char **)mallocx(count * sizeof(const char *));
...
void cleanup_previous_exercise(Tester *t)
{
    ...
    if (strlen(t->current_exercise_name) != 0)
    {
        ...
        if (t->filter_match && t->testcase_filter_match && t->successful)
        {
            ...
            fprintf(stderr, TESTRUNNER_LABEL ANSI_COLOR_GREEN "Erfolg %s" ANSI_COLOR_RESET "\n\n", happy_faces[t->current_exercise_number % NUM_HAPPY_FACES]);
        }
    }
}
...
bool begin_exercise(Tester *t, const char *name, const char *description)
{
    if (t->filter == NULL || strcmp(t->filter,name) == 0)
    {
        ...
        {
            fprintf(stderr, " in Testfall " ANSI_COLOR_YELLOW "%d" ANSI_COLOR_RESET "\n", t->testcase_filter);
        }
        else
        {
            fprintf(stderr, "\n");
...
void handle_assertion_failure(Tester *t, const char *expected, const char *got, const char *context)
{
    if (!t->should_skip_exercise && !t->should_skip_testcase && t->successful)
    {
        ...
        if (expected != NULL)
        {
            fprintf(stderr, TESTRUNNER_LABEL "Erwartet: %s\n", expected);
        }
        if (got != NULL)
        {
            fprintf(stderr, TESTRUNNER_LABEL "Erhielt:  %s\n\n", got);
        }
...
int wrap_up(Tester *t)
{
    ...
    if (t->filter == NULL)
    {
        ...
        {
            fprintf(stderr, TESTRUNNER_LABEL "Alle Tests bestanden!\n");
            fprintf(stderr, ANSI_COLOR_GREEN);
            for (int i = 0; i < NUM_HAPPY_FACES; i++)
            {
                fprintf(stderr, "%s  ", happy_faces[i]);
            }
        ...
        else
        {
            if (success_count > 0)
            {
                fprintf(stderr, TESTRUNNER_LABEL ANSI_COLOR_GREEN "Erfolge: " ANSI_COLOR_RESET "%zd/%zd\n", success_count, success_count + failure_count);
            }
            if (failure_count > 0)
            {
                fprintf(stderr, TESTRUNNER_LABEL ANSI_COLOR_CYAN "Baustellen: " ANSI_COLOR_RESET "%zd/%zd\n", failure_count, success_count + failure_count);
...
const char *format_uint16(uint16_t x) {
    int size = snprintf(NULL, 0, "%d", x);
    char* r = (char*) mallocx(size + 1);
    snprintf(r, size + 1, "%d", x);
    return r;
...
const char *format_size(size_t x) {
    int size = snprintf(NULL, 0, "%zu", x);
    char* r = (char*) mallocx(size + 1);
    snprintf(r, size + 1, "%zu", x);
    return r;
}
...
void assert_size(Tester *t, size_t expected, size_t got, const char *context) {
    if (expected != got) {
        const char* s_expected = format_size(expected);
        const char* s_got = format_size(got);
        handle_assertion_failure(t, s_expected, s_got, context);
        free((char*) s_expected);
        free((char*) s_got);
    }
...
void assert_canvas(Tester *t, Canvas_ *expected, Canvas_ *got, const char *context)
{
    ...
    char *name = (char *)mallocx(name_size);
    ...
    if (got->supplied_invalid_coordinates) {
        ...
        char *actual_context = (char *)mallocx(actual_context_size);
        ...
        char* filename = (char *)mallocx(filename_size);
        ...
        char* temp_name = mallocx(temp_len);
